...
# service's name and tier, and the repository's numeric GitHub IDs (which core's
# trust policy embeds, so a renamed or transferred repository cannot be
# impersonated). This prints the entry, ready to paste.
#
# The service name and tier are read from infrastructure/development/terraform.tfvars
# so they cannot disagree with what this repository deploys.
#
...
TFVARS="${REPO_ROOT}/infrastructure/development/terraform.tfvars"
[[ -f "${TFVARS}" ]] || { echo "ERROR: ${TFVARS} not found." >&2; exit 1; }

read_string() {
  sed -n "s/^[[:space:]]*$1[[:space:]]*=[[:space:]]*\"\([^\"]*\)\".*/\1/p" "${TFVARS}" | head -n 1
}

SERVICE="$(read_string service_name)"
TIER="$(read_string tier)"

if [[ -z "${SERVICE}" || "${SERVICE}" == "CHANGE_ME" ]]; then
  echo "ERROR: service_name is not set. Run scripts/init-service.sh first." >&2
  exit 1
fi
...
jq -n \
  ...
  '{($repo): {service_name: $service, kind: "infra", tier: $tier, owner_id: $owner, repository_id: $id}}'
