...
#
# .github/environments.json lists the environments this service runs in:
#
#   ["development", "production"]
#
# Any one, two or all three of development, staging and production, and only
# environments core runs too (a service has no platform where core has none).
# Folders of the others stay in the repository, ignored, so turning one on
# later is one line here (and scripts/init-service.sh). Every workflow asks
# this script, so nothing is planned or applied for an environment the service
# does not run.
#
# Usage:
...
case "${1:-}" in
  ...
  --check)
    ...
    if ! jq -e --arg e "${ENVIRONMENT}" 'index($e) != null' <<< "${ENABLED}" >/dev/null; then
      ...
      echo "       To run it, add it to .github/environments.json (scripts/init-service.sh --environments)." >&2
      exit 1
    fi
    ;;
  --filter)
