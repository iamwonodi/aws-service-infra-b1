...
#
# After core has applied this repository's service-roles.json entry, core
# publishes every role ARN to role-arns/<environment>.json on its
# platform-outputs branch. This reads this repository's ARN from there and sets
# it as the TF_AWS_ROLE_ARN secret on both GitHub Environments: <env> (apply) and
# <env>-plan (plan). Environment secrets are not shared between them.
#
...
for TARGET in "${ENVIRONMENT}" "${ENVIRONMENT}-plan"; do
  ...
  gh secret set TF_AWS_ROLE_ARN --repo "${REPO}" --env "${TARGET}" --body "${ARN}" \
    || { echo "ERROR: could not set the secret on ${TARGET}. Create the environments first: scripts/init-service.sh" >&2; exit 1; }
done
