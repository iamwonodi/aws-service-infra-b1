...
#
# This environment hosts services dedicated. Where the hosts go comes from the
# platform contract; how many and of what kind is set here.
################################################################################

variable "instance_types" {
  type        = list(string)
  default     = ["t3.small", "t3a.small"]
  description = "Instance types the hosts may use. More than one widens the pool a spot request can draw from."
}
