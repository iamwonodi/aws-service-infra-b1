...
| A service's own people (agents) are declared in its repository, `data/agents.json`, and reach only its database, as `<database>.<name>` | The team that owns the data decides who sees it; core's platform list covers people who need everything. The passwords and access levels go in the service's own secret, the one place core's provisioning can read in every environment (the managed databases' functions reach Secrets Manager only); the application never receives them, because its env file names only the entries it uses |
| Agents' emails reach the front door by declaration, `front-door/<service>.json`, which this repository's role may write and nothing else under that prefix | Cognito permissions cannot be limited to some of a pool's users; a core function turns every declaration into sign-ins. The declaration is written even when empty, so removing the last agent removes their sign-in |
| `agent-` is no longer a reserved service-name prefix; `database-` still is | People's logins contain a dot, which a service's database user never does |
